...
This skill is for **building apps that integrate Krea**. Not for one-off generations — that's the sibling `krea-ai` skill, which routes through the MCP server.
...
| Situation | Use |
|---|---|
| "Generate me an image of X" | `krea-ai` (MCP-based) |
...
1. **Prototype** — run the generation manually with `krea-ai` (MCP) to see what the actual output looks like.
