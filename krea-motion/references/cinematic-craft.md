...
This is the taste reference. The `krea-generate` skill's
`../../krea-generate/references/models/seedance-2.md` tells you the shape of a prompt and
`../references/cut-architecture.md` tells you how to structure the beats; this file
tells you what to put inside them so the result looks expensive instead of merely
generated.
...
One register legitimately inverts the darkness doctrine below: a beauty or skincare
product looping on a website, where the field is a tinted colour pulled from the
product and the shadow side falls to a deeper version of that hue rather than to
black. The cutting discipline is unchanged; the palette and light are not. If the
brief is a product page rather than a film, work from
`../references/product-beauty-macro.md` instead of this file's light and grade sections.
...
four beats in eight seconds, each from a new angle under a new key.
...
- Minimal beauty and web product loops, where the field is tinted rather than black
...
- The prompt's structural rules → `../../krea-generate/references/models/seedance-2.md`
