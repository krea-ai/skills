...
One case where a question is **mandatory** rather than optional: a product loop for a
website. Minimal and maximal are different films there — 0–2 cuts against 8–11, one
generation against four — and neither can be trimmed into the other. Ask before
building beats, using the wording in `../references/product-beauty-macro.md`.
...
1. `list_models` — confirm the live Seedance variant IDs.
2. `get_model_schema` for the variant you'll use.
3. Read `../../krea-generate/references/models/seedance-2.md` for field
   mapping and the mutually exclusive media paths.
...
- skincare, cosmetics, or any product loop for a website → `../references/product-beauty-macro.md`
- taste, light, lens, materials, tempo → `../references/cinematic-craft.md`

**One camera move per beat** — but four beats. Two moves inside one beat is
...
Build the prompt from `../../krea-generate/references/models/seedance-2.md`.
...
Two of these checks are inverted for a web product loop built from
`../references/product-beauty-macro.md`. If that register is the one in play, read the
bracketed variant instead — an alternating timeline and a zero-cut single take are
the specified output there, not failures.
...
- **Does the whole subject appear exactly once, on the last beat?** Seeing it in the
  ...
  the remaining time was padded. *[Beauty register, maximal mode: the whole product is
  supposed to appear on beat two and again at the end. Check instead that beat one is
  still a macro detail and that the two full views differ in scale and light.]*
- **Does the timeline progress, or does it alternate?** A full → detail → full
  pattern is a loop, not a build. *[Beauty register: alternation is the structure. Check
  instead that no two adjacent beats share a scale, an attitude or a key direction.]*
...
**Exempt: a prompt that declared a single continuous take.** The minimal mode and the
suspended cluster in `../references/product-beauty-macro.md` stage zero cuts on purpose,
so a detection count of zero is a pass there. What to check instead is the inverse
failure — *any* detected cut in a declared single take means Seedance invented one to
fill unstaged time, and that take is the one to restage.

### 9. Retake one thing

One change per retake, seed pinned. See the retake pattern in the `krea-generate`
skill's `../../krea-generate/references/models/seedance-2.md`, and diagnose from the failure table there
...
**A loaded register reference overrides this table**, and the rows it overrides are
always Environment, Light and Grade — those three encode the low-key void, which two
registers ban outright. `../references/luxury-showcase.md` replaces them with a designed
environment and a single warm accent; `../references/product-beauty-macro.md` replaces
them with a tinted field and a coloured shadow, and also overrides Structure, Duration
and Beat length. Take the row from the register, not from here, and do not treat
"animate my serum for the product page" as a brief that failed to say otherwise.

| Setting | Default |
|---|---|
| Aspect | 21:9 for cinematic, 16:9 for standard delivery, 9:16 for vertical |
| Duration | 8s; 6s for a logo sting; 4s floor |
| Structure | 4 staged beats, macro first, whole subject on the final beat |
...
- Do not answer "animate this" with one continuous move over the full duration. That
  is the timid-drift failure; build the beats. The one exception is the minimal mode in
  `../references/product-beauty-macro.md`, and only when the shot passes the five tests
  there.
- Do not open on the whole subject. The complete view is the last beat, not the first —
  except in the beauty register's maximal mode, where it lands on beat two by design.
