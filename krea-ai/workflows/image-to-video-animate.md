...
3. Resolve `image-to-video / start frame anchored` from live `list_models`.
...
generate_video(model="<image-to-video-model>", input={startImage, prompt, aspectRatio, duration}, sync=false)
get_job(jobId=<id>)
```
