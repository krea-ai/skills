...
5. Resolve live archetypes: `fast image draft`, `high-fidelity image`, `text in image / typography` if copy appears in image, and `faithful upscale` for winners.
6. Generate cheap drafts by angle and format first.
7. Read drafts with vision and reject outputs where product identity or claims are wrong.
8. Show a contact sheet or labeled list; let the user pick winners for final render/upscale.
9. Generate final winners through `product-photo-hero.md`, `product-photo-lifestyle.md`, `image-text-poster.md`, or `social-video-short.md` as needed.
10. **Deliver** organized outputs by platform, with QA notes and any unsupported claims removed.
