...
5. Call `get_model_schema(model=<id>)` for the candidate to confirm it accepts the inputs you have.
6. Submit.
...
**Schema hints.** Check whether the schema uses `imageUrl` (singular) or `imageUrls` (array). Multi-reference models (e.g. nano-banana-pro family) use `imageUrls` for face injection from several photos.

**Pattern.**
- One reference, simple edit → most image models work
- Multiple face references → look for models accepting `imageUrls` as an array
...
**Schema hints.** Most support `prompt`, `duration` (4-8s typical), `aspectRatio`.
...
**Schema hints.** Check for `resolution` (`720p`/`1080p`), `mode` (`std`/`pro`), `generateAudio`.
...
**Schema hints.** Some models expose `generateAudio: true` as a boolean. Others accept a reference audio file via an `audio` media input.
