...
- **Format**: offer talking-family formats from `../references/video-ad-formats.md` (ugc-native default; selfie-testimonial, direct-to-camera, problem-relief, hack-reveal, obsession-loop, couple-share, tutorial-steps, unboxing-spoken, tv-spot) or match one the user named.
...
4. **Storyboard.** Build one six-panel UGC storyboard sheet per `../references/ugc-social-video.md`, applying the realism rubric (texture, asymmetry, phone framing) and look preset, structured on the chosen format's device from `../references/video-ad-formats.md`. Loose brief -> 3 variants per `../references/storyboard-variations.md` (vary hook family + one other axis, or 3 formats from different families), A/B/C labels, wait for a pick. Resolve the storyboard image model from the marketing image set in `../SKILL.md`.
