...
own. Four land only when every beat is fully staged and no beat runs long. Cap beats at
2.5s and put `no long static holds, keep cutting` in the constraints tail. These are
beats inside one generation, so hard rule 3's lengths apply — see
...
These are separate assembled shots on Path B, so they may run longer than the
in-generation beat lengths in `../references/cut-architecture.md`. On Path A, use that
file's lengths instead.
...
| 4 — Accent (optional) | 1–1.5s | The fast one. Contrast makes the fuller beats read as deliberate. |
...
`list_models` → `get_model_schema` → the `krea-generate` skill's
`../../krea-generate/references/models/seedance-2.md` for field mapping.
...
Build the prompt per `../../krea-generate/references/models/seedance-2.md`.
...
```bash
ffmpeg -i sequence-raw.mp4 -vf "fps=1,scale=320:-1,tile=6x5" -frames:v 1 contact.png
```

The tile is 30 cells, so the sheet covers `30 ÷ fps` seconds — at `fps=1` that is 30s,
the top of this workflow's range. Set `fps` to at least `30 ÷ duration` or the sheet
stops before the landing. Read it with vision. Check: one world throughout, subject locked, marks intact in the
