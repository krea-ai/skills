...
The failure this prevents is structural, not a render-quality one: a logline, a six-scene storyboard and polished animation can still produce a piece whose story does not read.

## The spine (write all five before storyboarding)

Write a `Story spine` section into the story package you present in the conversation, with exactly these fields. One sentence each. If you can't write a sentence, you don't have a story.
...
The common failure is a silent film by accident — every scene storyboarded with an empty dialogue field. That's a default, not a choice. If you choose silent, justify it in the story package under a `Dialogue posture` line ("This piece is silent because the protagonist literally cannot speak — the bell-tones carry her voice").
...
If the user says "looks fine, proceed," push once: "Quick check — what's the protagonist's *want*? Confirming we both see the same story." If they can articulate it back, proceed. If they say "I don't know, you decide," you have a brief that will produce polished footage nobody can follow.

## Comprehensibility gate (the 5-year-old test)

A story whose spine is locked in your head but invisible on screen is a *failed* story. The characteristic verdict is "I see a random robot, random things, I don't understand anything" — on a piece whose premise, storyboard and shot list were all internally coherent. The story was in the plan, not in the pixels.

Before the story package is approved, answer all four of these in one sentence each. If you can't, the story is not yet comprehensible.
...
When unsure, default to one of these high-comprehension structures:
...
If the user's verdict on a previous attempt was that it did not read, identify the comprehensibility failure (it is almost always comprehensibility) and pick a new concept that fixes it. Do NOT iterate on the same story by adding more keyframes — that produces the same failure at higher cost. Iterate by choosing a more legible story.
